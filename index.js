...
client.on(Events.InteractionCreate, async (interaction) => {
    ...
    // --- Reaction Role Commands ---
        ...
        if (!config.reactionRoles[messageId]) return await interaction.editReply({ content: '❌ Message ID not found' });

        const channel = await client.channels.fetch(config.reactionRoles[messageId].channelId);
        const msg = await channel.messages.fetch(messageId);
        await msg.react(emojiInput);

        config.reactionRoles[messageId].emoji = emojiInput;
        config.reactionRoles[messageId].roleId = role.id;
        saveConfig();
        await interaction.editReply({ content: `✅ Added reaction ${emojiInput} for role ${role.name}` });
    } 
    else if (commandName === 'list-reactions') {
        let text = '📋 Reaction Role Messages:\n';
        for (const [msgId, data] of Object.entries(config.reactionRoles)) {
            text += `- Message ID: ${msgId}, Channel: <#${data.channelId}>, Emoji: ${data.emoji}, Role: <@&${data.roleId}>\n`;
        }
        if (Object.keys(config.reactionRoles).length === 0) text += 'None';
        await interaction.editReply({ content: text });
    } 
        ...
        saveConfig();
        await interaction.editReply({ content: `🗑️ Removed reaction role message ${messageId}` });
    }
});
